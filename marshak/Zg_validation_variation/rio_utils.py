...
from rasterio.windows import Window, transform as window_transform, from_bounds
...
def crop_to_bounds(array: np.ndarray, profile: dict, bounds: tuple[float, float, float, float], bounds_crs: str):
    target_bounds = transform_bounds(bounds_crs, profile["crs"], *bounds)

    window = from_bounds(*target_bounds, transform=profile["transform"])
    ...
    if array.ndim == 2:
        ...
        raise ValueError(f"Array must have 2 or 3 dimensions, got {array.ndim}")

    cropped_profile = profile.copy()
    cropped_profile["transform"] = profile["transform"] * profile["transform"].translation(col_off, row_off)
    cropped_profile["height"] = height
    cropped_profile["width"] = width
